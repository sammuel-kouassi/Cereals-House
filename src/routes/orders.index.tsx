...
const STATUS_STYLES: Record<string, { badge: string; label: string; icon: typeof Clock }> = {
  pending_payment: { badge: "bg-amber-500/15 text-amber-700 dark:text-amber-300", label: "En attente de paiement", icon: Clock },
  paid: { badge: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300", label: "Payée", icon: CheckCircle2 },
  preparing: { badge: "bg-blue-500/15 text-blue-700 dark:text-blue-300", label: "En préparation", icon: Package },
  shipped: { badge: "bg-indigo-500/15 text-indigo-700 dark:text-indigo-300", label: "Expédiée", icon: Truck },
  in_transit: { badge: "bg-purple-500/15 text-purple-700 dark:text-purple-300", label: "En cours de livraison", icon: Truck },
  delivered: { badge: "bg-emerald-600/20 text-emerald-700 dark:text-emerald-300", label: "Livrée", icon: CheckCircle2 },
  cancelled: { badge: "bg-destructive/15 text-destructive", label: "Annulée", icon: XCircle },
  refunded: { badge: "bg-muted text-muted-foreground", label: "Remboursée", icon: XCircle },
};
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-24 text-center sm:px-6">
        ...
        <Link
          ...
          className="mt-6 inline-flex rounded-full bg-gold px-6 py-3 text-sm font-semibold text-gold-foreground shadow-gold transition hover:bg-gold/90"
        >
          {t("orders.signInBtn", "Se connecter")}
        </Link>
      </div>
    );
  }
      ...
      {isLoading ? (
        ...
        <div className="mt-12 rounded-3xl border border-dashed border-border bg-card/50 p-12 text-center">
          ...
          <h3 className="font-display text-xl font-bold text-primary">
            {t("orders.emptyTitle", "Vous n'avez pas encore passé de commande")}
          </h3>
          <p className="mt-2 text-sm text-muted-foreground">
          ...
          <Link
            to="/products"
            className="mt-6 inline-flex items-center gap-2 rounded-full bg-gold px-6 py-2.5 text-xs font-semibold text-gold-foreground shadow-gold hover:bg-gold/90 transition"
          >
          ...
          {orders.map((order: any) => {
            ...
            return (
              <Link
                ...
                to="/orders/$id"
                params={{ id: order.id }}
                className="group flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-border bg-card p-5 transition hover:border-gold/40 hover:shadow-soft"
              >
